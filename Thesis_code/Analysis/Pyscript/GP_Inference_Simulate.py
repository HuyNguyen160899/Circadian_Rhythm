...
def CR_inference_mod(x = None, t = None, trials = 10, epochs = 100, debug=False, learning_rate=1e-2, period_start= 20, period_end = 24, disp = True):
    ...
    period     = np.arange(period_start, period_end + 1, 3) # Add a step for example 20 -> 22 -> 24
    res_per    = list()
...
def CR_inference_mod_2(x = None, t = None, trials = 10, epochs = 100, debug=False, learning_rate=1e-2, period_start= 20, period_end = 24, disp = True):
...
if __name__ == '__main__': 
    ...
    for x in range(len(data)):  
        gene  = np.array(data.iloc[x,1:], dtype = np.float32)
        plt.figure(figsize=(10,5))
        
        plt.subplot(1,2,1)
        param_1 = CR_inference_mod_2(disp= True, t = gene, trials= 10, period_start=24) 
        plt.xlabel("ZT (hours)")
        plt.ylabel("Normalized gene expression")
        plt.title("Null model of GP")
        
        plt.subplot(1, 2, 2)
        param_2 = CR_inference_mod(disp = True, t = gene, trials= 10,period_start=12)
        plt.xlabel("ZT (hours)")
        plt.ylabel("Normalized gene expression")
        plt.title("Circadian detection model with GP")
        
        plt.suptitle(f"Expression of gene {x}",fontsize=20)
        ...
        plt.show()
        plt.savefig(f"{out_dir}/Pics/Criteria_3_gene{x}.png")

    annot = np.array(data.iloc[:,0]) 
    cols = {'Period':p,'SNR':cri_1,'nSNR':cri_2,'LLR':cri_3,'0': annot}
    df   = pd.DataFrame(cols)
    df.to_csv(f"{out_dir}/GP_result_Criteria_3.csv")
