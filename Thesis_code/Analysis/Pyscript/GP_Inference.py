...
if __name__ == '__main__': 
    ...
    data   = pd.read_csv(filename) #data_2 = pd.read_excel('fixed_paper2.xlsx',sheet_name = 'retain_transcipts')
    data   = data.iloc[:,1:]
    annot  = data.iloc[:,0:3]
    ...
    for x in range(len(data)):  
        gene  = np.array(data.iloc[x,3:], dtype = np.float32).reshape([-1,3])
        x_arr = np.arange(1,72,3)
        ...
        plt.subplot(1, 2, 2)
        param_2 = CR_inference_mod(x = x_arr, disp = True, t = gene.T.flatten(), trials= 10,period_start=12)
        plt.xlabel("ZT (hours)")
        plt.ylabel("Normalized gene expression")
        plt.title("Circadian detection model with GP")
        
        plt.suptitle(f"Expression of gene {data.iloc[x,1]}",fontsize=20)
